...
public class SupplyHandler  {
    ...
    public static void requestResupplyIfNecessary(Robot robot) throws GameActionException {
        ...
        double travelTimeFromHQ = Math.sqrt(robot.rc.getLocation().distanceSquaredTo(robot.home));
        ...
        if (supplyRequestSize > robot.broadcast.readInt(BroadcastChannel.SUPPLY_MAX_NEEDED)) {
        	...
        	robot.broadcast.sendLocation(BroadcastChannel.MOST_NEEDED_SUPPLY_LOCATION, robot.rc.getLocation());
    ...
    public static void runSupplies(Robot robot) throws GameActionException {
        if (robot.rc.getLocation().equals(supplyRunnerLastLoc)) {
            supplyRunnerTurnsSinceMove++;
            if (supplyRunnerTurnsSinceMove >= 50 && robot.rc.getLocation().distanceSquaredTo(robot.home) > GameConstants.SUPPLY_TRANSFER_RADIUS_SQUARED) {
        ...
        supplyRunnerLastLoc = robot.rc.getLocation();
        ...
        if (supplyRunnerDest != null && robot.rc.getLocation().distanceSquaredTo(supplyRunnerDest) < 35) {
...
//        if (supplyRunnerDest != null)
//        Action.tryMove(robot.location.directionTo(supplyRunnerDest), robot.rc);
//        Action.tryMove(robot.location.directionTo(robot.getCheckpoint()), robot.rc);
        

//        NavSafetyPolicy safetyPolicy = new SafetyPolicyAvoidAllUnits(enemyTowers, nearbyEnemies);
        if (onSupplyRun) {
        	Action.tryMove(robot.rc.getLocation().directionTo(supplyRunnerDest), robot.rc);
//            Nav.goTo(supplyRunnerDest, safetyPolicy);
//             Debug.indicate("supply", 2, "going to supply dest");
            robot.rc.setIndicatorLine(robot.rc.getLocation(), supplyRunnerDest, 0, 255, 0);
        } else {
//            Nav.goTo(robot.home, safetyPolicy);
        	Action.tryMove(robot.rc.getLocation().directionTo(robot.home), robot.rc);
//             Debug.indicate("supply", 2, "returning to HQ");
    ...
    private static double supplyRunnerSpareSupplyAmount(Robot robot) {
        return robot.rc.getSupplyLevel() - 2 * RobotType.DRONE.supplyUpkeep * Math.sqrt(robot.rc.getLocation().distanceSquaredTo(robot.home));
    }
    ...
    private static boolean supplyRunnerRetreatIfNecessary(RobotInfo[] nearbyEnemies, MapLocation[] enemyTowers, Robot robot) throws GameActionException {
        if (!needToRetreat(nearbyEnemies, robot.rc.getLocation())) return false;

        Direction bestRetreatDir = null;
        RobotInfo currentClosestEnemy = Util.closest(nearbyEnemies, robot.rc.getLocation());
        ...
        int bestDistSq = robot.rc.getLocation().distanceSquaredTo(currentClosestEnemy.location);
        for (Direction dir : Direction.values()) {
            if (!robot.rc.canMove(dir)) continue;
            if (mustMoveOrthogonally && dir.isDiagonal()) continue;

            MapLocation retreatLoc = robot.rc.getLocation().add(dir);
